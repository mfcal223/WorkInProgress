...
/*   Updated: 2025/04/23 11:31:06 by mcalciat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	process_input(char *input, t_env *env)
{
	...
	if (detect_syntax_error(tokens, env))
	{
		free_tokens(tokens);
		return ;
	}
...
/* MAIN VERSION THAT INCLUDES BASIC PARSING TO BE ABLE TO TEST 
BUILTIN FUNCTIONS */
/*
 ...
 * NEEDS TO HANDLE ";" 
 * MULTIPLE/MIXED SEQUENCES echo hello | grep h ; echo world ; ls -l | wc -l
 * 
 void	execute_commands(t_cmd *cmd_list, t_env *env)
{
	while (cmd_list)
...
//USAR ESTA VERSION CON EL TESTER
