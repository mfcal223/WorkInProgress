...
/*   Updated: 2025/04/09 16:03:04 by mcalciat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cmd	*parse_cmd(char **tokens, int *i, t_env *env)
{
	...
	while (tokens[*i] && ft_strncmp(tokens[*i], "|", 1) != 0)
	{
		if (ft_strncmp(tokens[*i], "<<", 2) == 0
			|| ft_strncmp(tokens[*i], "<>", 2) == 0) //UNSUPPORTED REDIR (change when heredoc is added)
		{
		...
		ret = parse_redirection_token(cmd, tokens, i, env);
		if (ret == 0)
			return (NULL);
		if (ret == 2)
...
t_cmd	*parse_tokens(char **tokens,t_env *env)
{
	...
	while (tokens[i])
	{
		new_cmd = parse_cmd(tokens, &i, env);//included env
		if (!new_cmd)
		{
			free_cmd_list(head);
			return (NULL);
		...
		update_pipe_and_iterate(cmd, tokens, &i);//split into helper function - norminette
	}
	return (head);
}
