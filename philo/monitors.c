...
void	end_simulation(t_data *data)
{
	...
	data->dead = 1;
	pthread_mutex_unlock(&data->death_lock); // new position
...
void	*check_death(void *arg)
{
	...
	while (1)
	{
		...
		{
			/*printf("DEBUG: Death monitor ending because keep_iterating=%d,
				dead=%d\n",
		...
		while (i < data->num_philos)
		{
			if (has_died(&data->philo[i]))
			{
				/* printf("DEBUG: Philosopher %d death detected by monitor\n", i
					+ 1);*/
				end_simulation(data);
					// Need to call this here when death is detected
	...
	while (1)
	{
		...
		while (i < data->num_philos && !reset_check)
		{
			...
			if (data->philo[i].meals >= data->nb_meals)
				...
				reset_check = 1;//if 1 philo has not eat enough, then flag = true
